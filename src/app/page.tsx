...
// localStorage-backed state — hydrates after mount to avoid SSR mismatch
...
type DiscussionState = {
  dateISO: string; // ISO string so JSON-roundtrip works
  dateLabel: string;
  timeLabel: string;
  location: string;
};
...
export default function LibraryDashboard() {
  ...
  const [pedroVotes, setPedroVotes] = useLocalStorage<string[]>("pbc-votes-p", []);
  const [lauraVotes, setLauraVotes] = useLocalStorage<string[]>("pbc-votes-l", ["t2"]);
  const [votingAs, setVotingAs] = useLocalStorage<"p" | "l">("pbc-voting-as", "p");
  const [readBookA, setReadBookA] = useLocalStorage<number>("pbc-read-a", books[0].current);
  const [readBookB, setReadBookB] = useLocalStorage<number>("pbc-read-b", books[1].current);
  const [notesList, setNotesList] = useLocalStorage<NoteEntry[]>("pbc-notes", notesData);
  const [topics, setTopics] = useLocalStorage<NextTopic[]>("pbc-topics", nextTopics);
  const [discPrompts, setDiscPrompts] = useLocalStorage<string[]>("pbc-prompts", promptsSeed);
  const [monthState, setMonthState] = useLocalStorage<MonthState>("pbc-month", monthSeed);
  const [discState, setDiscState] = useLocalStorage<DiscussionState>("pbc-disc", {
  ...
  const noteCounts = notesList.reduce<{ a: number; b: number }>(
    (acc, n) => {
      // note.who 0 = Pedro (reads bookB), 1 = Laura (reads bookA)
      if (n.who === 0) acc.b += 1;
      else acc.a += 1;
      return acc;
    },
  ...
  return (
    ...
    >
        ...
        >
          ...
          <div>
            <div
              ...
              style={{ fontSize: 22, color: PALETTE.cream, lineHeight: 0.9 }}
            >
              Pookers
            </div>
            <div
              className="pbc-display"
              style={{ fontSize: 22, color: PALETTE.peach, lineHeight: 0.9 }}
            >
              bookclub
            </div>
          </div>
      ...
      <main style={{ padding: "32px 40px 48px", overflow: "hidden" }}>
        ...
        {tab === "us" && (
          <FocusWrap title="The two readers" eyebrow="the pooks behind the shelf">
            <ReadersView />
          </FocusWrap>
        )}
      </main>
    ...
    >
      <div
        style={{
          ...
          boxShadow:
            "0 24px 50px rgba(45,31,21,.30), 0 0 0 1px rgba(45,31,21,.08)",
          position: "relative",
          justifySelf: "center",
          width: 280,
        }}
      ...
      <div>
        ...
        >
          ...
          <div style={{ marginLeft: -10 }}>
            <Avatar reader={readers[1]} size={32} ring />
          </div>
          <div
            style={{
              fontSize: 13,
...
function CountdownTile({
  ...
  return (
    ...
    >
      ...
      <div
        className="pbc-display"
        style={{ fontSize: 64, color: PALETTE.teal, lineHeight: 0.9 }}
      >
        {days}
      </div>
      <div>
        {editing ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <input
              type="datetime-local"
              value={toDateTimeLocalValue(discState.dateISO)}
            ...
            />
            <input
              value={discState.location}
              onChange={(e) =>
                setDiscState({ ...discState, location: e.target.value })
              }
          ...
          <>
            <div
              style={{ fontSize: 13, color: PALETTE.espresso, fontWeight: 700 }}
            >
              days · {discState.dateLabel}
            </div>
    ...
    >
      ...
      <div style={{ color: accent }}>
        ...
        >
          ...
          <span>
            {book.pages - book.current} pages left · ~
            {Math.ceil((book.pages - book.current) / 30)} sittings
          </span>
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div>
      <div
        className="pbc-display"
        style={{ fontSize: 20, color: PALETTE.espresso, lineHeight: 1 }}
      >
        {value}
      </div>
      <div
    ...
    >
      ...
      >
        <div>
          ...
          <div
            className="pbc-display"
            style={{ fontSize: 28, color: PALETTE.espresso, marginTop: -2 }}
          >
        ...
        <button
          onClick={() => setOpen((v) => !v)}
          style={{
            background: open ? PALETTE.taupeDark : PALETTE.espresso,
            color: PALETTE.cream,
            border: "none",
            borderRadius: 999,
            padding: "8px 16px",
      ...
      {open && (
        <div
          style={{
            ...
            flexDirection: "column",
            gap: 10,
            border: `1.5px solid ${PALETTE.taupe}`,
          }}
        >
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    color: on ? "#fff" : PALETTE.espresso,
                    border: `1.5px solid ${on ? r.color : "rgba(45,31,21,.2)"}`,
                    borderRadius: 999,
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            ...
            <button
              ...
              disabled={!text.trim()}
              style={{
                background: text.trim() ? PALETTE.espresso : "rgba(45,31,21,.3)",
                color: PALETTE.cream,
                border: "none",
                borderRadius: 999,
                padding: "6px 16px",
      ...
      {notes.length === 0 ? (
        ...
        >
          <div className="pbc-hand" style={{ fontSize: 24, color: PALETTE.midBrown }}>
            no notes yet — first page, fresh page.
          </div>
          <div
            style={{
              fontSize: 12,
...
function VotePanel({
  ...
  const updateTopic = (id: string, patch: Partial<NextTopic>) =>
    ...
    <div style={{ background: PALETTE.cream, borderRadius: 20, padding: 22 }}>
      ...
      >
        <div>
          ...
          >
            pick as many as you like — tap to toggle
          </div>
        </div>
          ...
          >
            {[Pedro, Laura].map((r) => {
              ...
              return (
                <button
                  key={r.id}
                  onClick={() => setVotingAs(r.id === "p" ? "p" : "l")}
                  style={{
                    display: "flex",
                    alignItems: "center",
                    gap: 6,
                    background: isMe ? r.color : "transparent",
                    color: isMe ? "#fff" : PALETTE.espresso,
                    border: "none",
                    borderRadius: 999,
                ...
                >
                  <Avatar reader={r} size={22} />
                  {r.name}
                  <span
                    style={{
                      background: isMe ? "rgba(255,255,255,.25)" : "rgba(45,31,21,.08)",
                      borderRadius: 999,
                      padding: "1px 7px",
                      fontSize: 11,
                      fontWeight: 800,
          ...
          return (
            <div
              ...
              style={{
                ...
                overflow: "hidden",
                background: PALETTE.paper,
                border: `2px solid ${youVoted ? PALETTE.taupe : "transparent"}`,
              }}
            >
              <div
                style={{
      ...
      {addOpen ? (
        ...
        >
          ...
          <button
            ...
            style={{
              background: newLabel.trim() ? PALETTE.espresso : "rgba(45,31,21,.3)",
              color: PALETTE.cream,
              border: "none",
              borderRadius: 999,
              padding: "5px 14px",
      ...
      >
        winner gets picked on {dateLabel} after discussion
      </div>
    </div>
  );
}
...
function VoteChip({ reader, you }: { reader: Reader; you: boolean }) {
  return (
    <span
      title={`${reader.name} voted${you ? " (you)" : ""}`}
      style={{
        display: "inline-flex",
        alignItems: "center",
        gap: 4,
        ...
        letterSpacing: ".06em",
        textTransform: "uppercase",
        boxShadow: you ? `0 0 0 2px ${PALETTE.paper}, 0 0 0 3px ${reader.color}` : "none",
      }}
    >
      <span
        style={{
    ...
    >
      ...
      >
        <div>
          ...
          <div
            className="pbc-display"
            style={{ fontSize: 28, color: PALETTE.cream, marginTop: -2 }}
          >
            our reading history
          </div>
        </div>
        <span style={{ fontSize: 13, opacity: 0.8 }}>
          0 finished · 1 in progress · ∞ to go
        </span>
      </div>
      <div
        style={{
      ...
      >
        {shelfMonths.map((m, i) => (
          <div
            ...
            style={{
              background:
                m.filled === "current" ? PALETTE.cream : "rgba(0,0,0,.18)",
              color:
                m.filled === "current"
                  ? PALETTE.espresso
                  : "rgba(242,234,224,.5)",
          ...
          >
            ...
            {m.filled === "current" ? (
              ...
              <div className="pbc-hand" style={{ fontSize: 20, marginTop: 12 }}>
                tbd
              </div>
            )}
          </div>
      ...
      <div style={{ flex: 1, minWidth: 320 }}>
        ...
        >
          ...
          <button
            ...
            title={editing ? "done" : "edit month"}
            aria-label={editing ? "done editing" : "edit month"}
          >
            {editing ? "✓" : "✎"}
          </button>
        </div>
        {editing ? (
          ...
          >
            <input
              value={monthState.topic}
              onChange={(e) =>
                setMonthState({ ...monthState, topic: e.target.value })
              }
              placeholder="month topic"
              style={{
            ...
            />
            <textarea
              value={monthState.blurb}
              onChange={(e) =>
                setMonthState({ ...monthState, blurb: e.target.value })
              }
              rows={3}
...
function DiscussionView({
  ...
  const updatePrompt = (i: number, v: string) =>
    setDiscPrompts((prev) => prev.map((p, idx) => (idx === i ? v : p)));
  const removePrompt = (i: number) =>
    setDiscPrompts((prev) => prev.filter((_, idx) => idx !== i));
        ...
        {editHero ? (
          ...
          >
            <input
              ...
              style={{
                border: "1.5px solid rgba(242,234,224,.3)",
                borderRadius: 10,
                padding: "8px 12px",
          ...
          <>
            ...
            >
              {discState.timeLabel} · {discState.location}
            </div>
          </>
        )}
        <div
      ...
      >
        ...
        >
          ...
          <button
            ...
            style={{
              background: newPrompt.trim() ? PALETTE.espresso : "rgba(45,31,21,.3)",
              color: PALETTE.cream,
              border: "none",
              borderRadius: 999,
              padding: "8px 16px",
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderRadius: 14,
                  padding: "16px 18px",
                  borderLeft: `4px solid ${PALETTE.peach}`,
                  position: "relative",
                }}
              >
                <div
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: 4 }}>
                    <button
                      ...
                      aria-label={editing ? "done editing" : "edit prompt"}
                      style={{
                        background: "transparent",
                        border: "none",
                        color: PALETTE.taupeDark,
    ...
    >
      {readers.map((r) => (
        ...
        >
          ...
          >
            picks for <strong>{r.pickedFor}</strong>. trusts {r.pickedFor} to
            pick well right back.
          </div>
        </div>
      ))}
    </div>
